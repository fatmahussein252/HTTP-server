...
void send_directory_listing(DIR* dir, int client_socket, const char *directory_path) {
    char response[RES_SIZE];
    snprintf(response, sizeof(response),
     	"HTTP/1.1 200 OK\r\n"
        "Content-Type: text/html\r\n"
        "\r\n"
        "<html><body><h1>Directory Listing of: %s</h1><ul>",directory_path);
    // Add directory entries
     if (dir == NULL) {
	printf("OPEN: open directory failed (%s)\n%s", strerror(errno),directory_path);
	exit(1);
	}
    struct dirent *entry;
...
void cat_file(int fd, int client_socket,char *file_path) {
    char response[RES_SIZE];
    char buf[BUF_SIZE];
    snprintf(response, sizeof(response),
     	"HTTP/1.1 200 OK\r\n"
...
void send_error(int cfd ,int status_code, const char *reason, const char *msg) {
    char response[RES_SIZE];
    snprintf(response, RES_SIZE,
        ...
        "</html>",status_code, reason, status_code, reason, status_code, reason, msg);

    write(cfd, response, strlen(response));
}

void run_cgi_script(int cfd, const char *path, const char *query) {
    pid_t pid;
    int pipefd[2];
    ...
    if (access(script_path, X_OK) != 0) {
        send_error(cfd, 404, "Not Found", "CGI script not found or not executable.");
        return;
    }
	...
	dup2(STDOUT_FILENO, outfd); // Redirect script output to pipe
        ...
        perror("fork");
        send_error(cfd, 500, "Internal Server Error", "Failed to execute CGI script.");
    }
}
int main(int argc, char *argv[])
{
	...
	for (;;) {		/* Handle client connections iteratively */
		...
		if (cpbuf == NULL)
		send_error(cfd,405, "Method Not supported", "Only GET method is supported.");
		else{
		strtok(cpbuf, " ");
		request = strtok(NULL, " ");
		...
		send_error(cfd, 404, "Not Found", "The requested resource was not found on this server.");
		}
